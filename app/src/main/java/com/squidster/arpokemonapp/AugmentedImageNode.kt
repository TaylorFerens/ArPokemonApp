package com.squidster.arpokemonapp
...
class AugmentedImageNode(
    ...
    private var modelCompletableFuture = ModelRenderable.builder()
        .setSource(context, R.raw.beeddrill)
        .build()
    ...
    fun setAugmentedImage(image: AugmentedImage){
        this.image = image
        // Check that the model is done rendering
        ...
        // AugmentedImageNode is the target image
        val modelNode = Node().apply{
            setParent(this@AugmentedImageNode)
            renderable = this@AugmentedImageNode.renderable
        }

        // Now we need to scale the 3D renderable to the size of the target
